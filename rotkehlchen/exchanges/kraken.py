...
class Kraken(ExchangeInterface, ExchangeWithExtras, SignatureGeneratorMixin):
    ...
    def process_futures_account_log(self, logs: list[dict[str, Any]]) -> list[HistoryBaseEntry]:
        ...
        for entry in logs:
            ...
            if info in ('transfer', 'admin transfer'):
                ...
                events.extend(create_asset_movement_with_fee(
                    location=Location.KRAKEN,
                    event_subtype=HistoryEventSubType.RECEIVE if amount > ZERO else HistoryEventSubType.SPEND,
                    unique_id=entry.get('booking_uid', ''),
                    asset=asset,
                    amount=abs(amount),
                    fee=AssetAmount(asset=asset, amount=ZERO),
                    timestamp=timestamp,
                    location_label=self.name,
                    movement_notes=f"Kraken Futures {info}",
                ))
                continue
    ...
    def _process_futures_trade_entries(self, execution_id: str, entries: list[dict[str, Any]]) -> list[SwapEvent]:
        ...
        try:
            ...
            is_liquidation = info in ('futures liquidation', 'futures assignor')
