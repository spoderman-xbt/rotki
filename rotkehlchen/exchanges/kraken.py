...
class Kraken(ExchangeInterface, ExchangeWithExtras, SignatureGeneratorMixin):
    ...
    def process_futures_account_log(self, logs: list[dict[str, Any]]) -> list[HistoryBaseEntry]:
        ...
        for entry in logs:
            ...
            if execution_id:
                ...
                # Funding don't have execution ID
    ...
    def _process_futures_log_entry(self, entry: dict[str, Any]) -> HistoryEvent | None:
        """Process a single non-trade log entry (funding)"""
    def _process_futures_trade_entries(self, execution_id: str, entries: list[dict[str, Any]]) -> list[SwapEvent]:
        ...
        try:
            ...
            for entry in entries:
                ...
                if contract.startswith('pi_'):
                    return []  # TODO: Single collateral future entries are not supported
            ...
            if not base_entry or not quote_entry:
                ...
                log.error(msg)
                # return []
                raise UnprocessableTradePair(contract)
            ...
            quote_asset_str = quote_entry['asset'].upper()
            quote_asset = asset_from_kraken(quote_asset_str)
            fee_asset = quote_asset  # Fee is always paid in quote asset
